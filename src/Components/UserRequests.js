...
const UserRequests = () => {
  ...
  const rows = requests.map((item, index) => ({
    id: index + 1,
    userName: item.userId ? `${item.user.fname} ${item.user.lname}` : "N/A", // Concatenate first name and last name
    docName: item.documentId ? item.document.docName : "N/A",
    docNumber: item.documentId ? item.documentId : "N/A",
    version: item.documentId ? item.document.version : "N/A",
    remarks: item.document.remarks || "N/A", // You may replace this with the actual field from the API if available
